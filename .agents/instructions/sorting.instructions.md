...
This section gives instructions specifically for HelmReleases that are based on the `app-template` chart. These can be identified by checking if the `spec.chartRef.name` field within the HelmRelease is set to `app-template`.
...
1. Check the `spec.chartRef.name` field within the HelmRelease.
2. Confirm that the `name` is `app-template`.
